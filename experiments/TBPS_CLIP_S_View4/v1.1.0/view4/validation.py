...
def check_e0_checkpoint(path, cfg, inputs):
    ...
    expected = ("E0", 1, "formal", "complete", 5)
    found = tuple(result.get(k) for k in ("experiment", "seed", "run_kind", "status", "epochs"))
    if found != expected:
        raise ValueError("E0 must have completed the five-epoch S reproduction")
    ...
    return {"file": file_record(path), "best_epoch": saved["best"]["epoch"],
