...
def train(cfg, experiment, seed, run_kind, output_dir, command, e0_checkpoint=None, resume=None):
    ...
    if seed not in cfg["seeds"] or (experiment == "E0" and (seed != 1 or run_kind != "formal")):
    ...
    def create_record():
        ...
        if not resume:
            write_json(out / "config.json", dict(cfg, experiment=experiment, seed=seed, run_kind=run_kind,
                       e0_checkpoint=str(e0_checkpoint), num_epochs=5 if run_kind == "formal" else 2))
    ...
    optimizer, optimizer_audit = build_optimizer(model, lr_value(0, experiment))
    ...
    limit = 5 if run_kind == "formal" else 2
    ...
    try:
        while epoch <= limit:
            ...
            for step, host in enumerate(iterator, start=start_step):
                ...
                global_step = (epoch - 1) * 212 + step
                lr = lr_value(global_step, experiment)
                for group in optimizer.param_groups:
                    group["lr"] = lr * group["ratio"]
                beta, lam = loss_weights(epoch, step, experiment)
                ...
                if rank() == 0:
                    ...
                    log.flush()
                    if step % 10 == 0 or step == 211:
                        line = "Epoch[%d] Iter[%d/212] loss=%.5f nitc=%.5f ritc=%.5f pairs=%d lambda=%.6f skip=%s" % (
                            epoch, step + 1, metrics["loss_total"], metrics["nitc"], metrics["ritc"],
        ...
        del best_saved
        # Smoke never evaluates test. Formal test runs exactly once, at the validation-selected checkpoint.
        testing = rank0_call(lambda: evaluate(model, catalog["test"], cfg["dataset_root"], device), control) if run_kind == "formal" else None
