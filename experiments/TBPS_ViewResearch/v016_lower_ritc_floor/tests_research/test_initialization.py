...
class InitializationTests(unittest.TestCase):
    ...
    def test_only_new_expert_parameters_may_be_missing(self):
        cfg = dict(seed=1, ritc_eps=.0001, initialization='openai_clip', clip_checkpoint=str(Path('openai.pt').resolve()))
            ...
            with patch('research.model.SoftViewCLIP', return_value=model), \
                ...
                if bad:
                    with self.assertRaises(RuntimeError):
                        build_model(SimpleNamespace(experiment=SimpleNamespace()), cfg, 4, cfg['clip_checkpoint'])
                else:
                    official = SimpleNamespace(experiment=SimpleNamespace(ritc_eps=.01))
                    loaded = build_model(official, cfg, 4, cfg['clip_checkpoint'])
                    self.assertEqual(official.experiment.ritc_eps, .0001)
    ...
    def test_e0_checkpoint_cannot_be_used_as_initializer(self):
        ...
        with patch('research.model.SoftViewCLIP', return_value=self.tiny()), \
            ...
            with self.assertRaises(ValueError):
                build_model(SimpleNamespace(experiment=SimpleNamespace()), cfg, 4, str(Path('best.pth').resolve()))
            loader.assert_not_called()

    def test_floor_cannot_be_silently_ignored_or_changed(self):
        official = SimpleNamespace(experiment=SimpleNamespace(ritc_eps=.01))
        with self.assertRaises(ValueError):
            build_model(official, dict(ritc_eps=.01), 4)
        model = self.tiny()
        model.eps = .01
        with patch('research.model.SoftViewCLIP', return_value=model), \
                patch('research.model.install_checkpoint_compatibility'):
            with self.assertRaises(RuntimeError):
                build_model(official, dict(ritc_eps=.0001, seed=1), 4)
