...
def load_config(path):
    ...
    if (cfg["world_size"], cfg["local_batch"], cfg["steps_per_epoch"],
            cfg["P"], cfg["K"]) != (4, 80, 115, 160, 2):
        raise ValueError("RSTP seed1 setup is fixed to 4x80, 115 steps, P160 K2")
    if cfg.get("dataset") != "RSTPReid" or cfg["epochs"] != 5:
        raise ValueError("Only five-epoch RSTP E0 is enabled in this snapshot")
    if (cfg["alpha"], cfg["delta"], cfg["lambda_max"], cfg["seeds"]) != (0.2, 60, 0.01, [1]):
        raise ValueError("RSTP seed1 constants cannot be silently overridden")
...
def lr_value(step, experiment, epochs=5, steps_per_epoch=115):
...
def loss_weights(epoch, step, experiment, steps_per_epoch=115):
