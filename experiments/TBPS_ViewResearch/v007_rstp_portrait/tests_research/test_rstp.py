...
class RSTPTests(unittest.TestCase):
    ...
    def test_baseline_verification_rejects_test_and_bad_steps(self):
        ...
        result = dict(experiment='E0',seed=1,epochs=5,status='complete',config={'dataset':'RSTPReid','input_resolution':[384,128]},
    ...
    def test_continuation_requires_own_measured_e0(self):
        ...
        cfg = json.loads((root/'configs/research_v007.json').read_text())
        baseline = dict(status='complete',experiment='E0',seed=1,epochs=5,
                        config={'dataset':'RSTPReid','input_resolution':[384,128]},test_at_best_validation=None,
