...
class AllocationTests(unittest.TestCase):
    def test_peak_group_rates_and_unchanged_auxiliary_schedule(self):
        ...
        cfg = read_json(Path(__file__).resolve().parents[1] / 'configs/research_v006.json')
        lr, _, _, dec = schedule(cfg, 1, 114)
        self.assertAlmostEqual(lr, 1e-5)
        self.assertAlmostEqual(lr * cfg['expert_lr_peak'] / cfg['base_lr_peak'], 1e-3)
        self.assertEqual(dec, 0.)
        _, _, shared, dec = schedule(cfg, 2, 114)
