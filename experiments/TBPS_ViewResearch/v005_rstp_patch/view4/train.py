...
def train(cfg, experiment, seed, run_kind, output_dir, command, e0_checkpoint=None, resume=None, audit_steps=None):
    if experiment != "E0":
        raise ValueError("Use the research entry for RSTP soft-view continuation")
    if audit_steps is not None and (audit_steps != 4 or resume):
        raise ValueError("E0 audit is four fresh steps only")
                ...
                if audit_steps and step + 1 >= audit_steps:
                    ...
                    if rank() == 0:
                        write_json(out / "audit_result.json", dict(status="passed", steps=audit_steps,
                                   telemetry=epoch_stats, synchronization=sync, test_evaluated=False))
