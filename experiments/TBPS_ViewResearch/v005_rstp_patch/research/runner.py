...
from view4.common import (ROOT, manifest, rank, read_json, load_training_checkpoint,
...
def load_config(path):
    cfg = read_json(path)
    if cfg["version"] != VERSION or cfg["dataset"] != "RSTPReid":
        raise ValueError("This immutable version is the RSTP soft3 experiment only")
    if (cfg["world_size"], cfg["local_batch"], cfg["steps_per_epoch"], cfg["seed"], cfg["seeds"]) != (4, 80, 115, 1, [1]):
        raise ValueError("Fixed four-GPU single-seed caption-pair setup")
    if cfg["main_sampler"] != "E0_caption_pair" or cfg["epochs"] != 5:
        raise ValueError("Five additional epochs with the E0 caption-pair sampler required")
    for key in ("dataset_root", "annotation_root", "orientation", "prepared_dir", "clip_checkpoint", "initializer", "nltk_data"):
    ...
    baseline = read_json(Path(cfg["initializer"]).parents[1] / "result.json")
    if (baseline["status"], baseline["experiment"], baseline["seed"], baseline["epochs"]) != ("complete", "E0", 1, 5):
        raise ValueError("Measure the completed RSTP E0 before continuing")
    if baseline["config"]["dataset"] != "RSTPReid" or baseline["test_at_best_validation"] is not None:
        raise ValueError("Expected a validation-only RSTP baseline")
...
def telemetry(output, batch):
    ...
    return dict(zip(keys, values.tolist()), shared_mean=float(output["shared_mean"]),
                ...
                distinct_pids=int(torch.cat(ids).unique().numel()), raw_views=views[:3].tolist(),
...
def train(cfg, output_dir, command, resume=None, audit_steps=None):
    ...
    def record():
        ...
        disk_guard(out, 1800 * 1024**2)
        if not resume:
            write_json(out / "config.json", cfg)
            write_json(out / "run_manifest.json", dict(manifest(command, [cfg["initializer"], cfg["orientation"],
                       Path(cfg["prepared_dir"]) / "catalog.json"], out, cfg["seed"]),
                       experiment=VERSION, evaluation_split="val_only", orientation_calibrated=False))
        return utcnow().replace(":", "").replace(".", "_")

    attempt = rank0_call(record, control)
    catalog, sources = enriched_catalog(cfg)
    initial = rank0_call(lambda: check_e0_checkpoint(cfg["initializer"], cfg, sources), control)
    ...
    try:
        say("START %s seed=1 main=caption-pair val-only auxiliary_train_images_only" % VERSION)
        while epoch <= cfg["epochs"]:
            plan = Path(cfg["prepared_dir"]) / "plans/seed_1" / ("E0_epoch_%d.npz" % epoch)
                ...
                if audit_steps and step + 1 >= audit_steps:
                    ...
                    if rank() == 0:
                        write_json(out / "audit_result.json", dict(status="passed", steps=audit_steps,
                                   telemetry=batches, synchronization=synchronization, test_evaluated=False))
...
def verify_result(path):
    ...
    for row in result["history"]:
        if row["batches"] != 115 or not all(math.isfinite(row["validation"][k]) for k in ("r1", "mAP")):
            raise RuntimeError("Missing batches or nonfinite validation")
    print(json.dumps({"guard": "passed", "metric": result["joint_validation_gain_pp"],
