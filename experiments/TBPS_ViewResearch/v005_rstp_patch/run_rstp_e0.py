...
def verify(result):
    if (result['experiment'], result['seed'], result['epochs'], result['status']) != ('E0', 1, 5, 'complete'):
        raise RuntimeError('Incomplete RSTP E0')
    if result['config']['dataset'] != 'RSTPReid' or result['test_at_best_validation'] is not None:
        raise RuntimeError('Baseline must be RSTP validation-only')
...
def main():
    ...
    for name in ('prepare', 'train', 'audit'):
        ...
        if name == 'audit':
            s.add_argument('--steps', type=int, choices=[4], default=4)
