...
def verify(result):
    ...
    if (result['experiment'], result['seed'], result['epochs'], result['status']) != ('E0', 1, 5, 'complete'):
        raise RuntimeError('Incomplete ICFG E0')
    if result['config']['dataset'] != 'ICFG-PEDES' or result['test_at_best_validation'] is not None:
        raise RuntimeError('Baseline must be ICFG validation-only')
