...
from .icfg_split import original_split, verify_derived

EXPECTED = {"train": (31289, 2792, 31289), "val": (3383, 310, 3383), "test": (19848, 1000, 19848)}
...
def prepare(cfg, output_dir, command):
    ...
    for split in EXPECTED:
        ...
        for ann in annotations:
            ...
            source_split = original_split(split, ann)
            if not o or o["status"] != "ok" or source_split not in o["splits"] or ann["id"] not in o["pids"]:
        ...
        if actual != EXPECTED[split]:
            raise ValueError("Unexpected ICFG counts: %s %s" % (split, actual))
    ...
    for seed in cfg["seeds"]:
        ...
        for epoch in range(1, cfg["epochs"] + 1):
            pk, stats = pk_plan(catalog["train"], seed, epoch, steps=cfg["steps_per_epoch"],
                                world_size=cfg["world_size"], p=cfg["P"], k=cfg["K"])
            for kind, plan in (("PK", pk), ("E0", e0_plan(EXPECTED["train"][2], epoch,
                                steps=cfg["steps_per_epoch"], world_size=cfg["world_size"], local_batch=cfg["local_batch"]))):
    ...
    write_json(out / "summary.json", {"counts": EXPECTED, "num_train_ids": EXPECTED["train"][1],
...
def source_paths(cfg):
    return {**{split: str((Path(cfg["annotation_root"]) / (split + "_reid.json")).resolve())
               for split in EXPECTED},
            **{'official_'+split: str((Path(cfg['source_annotation_root'])/(split+'_reid.json')).resolve())
               for split in EXPECTED},
            'split_manifest': str((Path(cfg['annotation_root'])/'split_manifest.json').resolve()),
    ...
    for split, expected in EXPECTED.items():
        ...
        for row, ann in zip(rows, annotations):
            ...
            if (row["image_path"] != relative or row["original_pid"] != pid
                    ...
                    or row.get("captions_bt", []) != bt or
                    row.get('source_split') != original_split(split, ann)):
            ...
            if (not a or a["status"] != "ok" or pid not in a["pids"] or
                    original_split(split, row) not in a["splits"]):
