...
def load_config(path):
    ...
    required = {"dataset_root", "annotation_root", "source_annotation_root", "orientation", "clip_checkpoint", "prepared_dir",
    ...
    if (cfg["world_size"], cfg["local_batch"], cfg["steps_per_epoch"],
            cfg["P"], cfg["K"]) != (4, 80, 97, 160, 2):
        raise ValueError("ICFG seed1 setup is fixed to 4x80, 97 steps, P160 K2")
    if cfg.get("dataset") != "ICFG-PEDES" or cfg["epochs"] != 5:
        raise ValueError("Only five-epoch ICFG portrait E0 is enabled in this snapshot")
    if (cfg["alpha"], cfg["delta"], cfg["lambda_max"], cfg["seeds"]) != (0.2, 60, 0.01, [1]):
        raise ValueError("ICFG seed1 constants cannot be silently overridden")
    for key in ("dataset_root", "annotation_root", "source_annotation_root", "orientation", "clip_checkpoint", "prepared_dir", "nltk_data"):
