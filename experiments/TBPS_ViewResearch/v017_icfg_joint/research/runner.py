...
def load_config(path):
    cfg = read_json(path)
    check_geometry(cfg)
    if cfg["version"] != VERSION or cfg["dataset"] != "ICFG-PEDES":
        raise ValueError("This immutable version is the ICFG initial joint-training experiment only")
    if (cfg["world_size"], cfg["local_batch"], cfg["steps_per_epoch"], cfg["seed"], cfg["seeds"]) != (4, 80, 97, 1, [1]):
    ...
    if cfg['ritc_eps'] != .001:
        raise ValueError('V017 fixes the R-ITC target floor to 0.001')
    for key in ("dataset_root", "annotation_root", "source_annotation_root", "orientation", "prepared_dir", "clip_checkpoint", "initializer", "baseline_checkpoint", "nltk_data"):
    ...
    if (baseline["status"], baseline["experiment"], baseline["seed"], baseline["epochs"]) != ("complete", "E0", 1, 5):
        raise ValueError("Measure the completed ICFG portrait E0 before the experiment")
    if baseline["config"]["dataset"] != "ICFG-PEDES" or baseline["test_at_best_validation"] is not None:
        raise ValueError("Expected a validation-only ICFG portrait baseline")
...
def verify_result(path):
    ...
    for row in result["history"]:
        if row["batches"] != 97 or not all(math.isfinite(row["validation"][k]) for k in ("r1", "mAP")):
