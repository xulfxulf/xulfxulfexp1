...
def load_config(path):
    ...
    if cfg["main_sampler"] != "E0_caption_pair" or cfg["epochs"] != 10:
        raise ValueError("Ten epochs from OpenAI with the E0 caption-pair sampler required")
    ...
    if (baseline["status"], baseline["experiment"], baseline["seed"], baseline["epochs"]) != ("complete", "E0", 1, 10):
...
def train(cfg, output_dir, command, resume=None, audit_steps=None):
    ...
    try:
        say("START %s init=OpenAI seed=1 main=caption-pair val-only auxiliary_train_images_only" % VERSION)
...
def verify_result(path):
    ...
    if result["status"] != "complete" or result["epochs"] != 10 or len(result["history"]) != 10:
        raise RuntimeError("Incomplete ten-epoch experiment")
