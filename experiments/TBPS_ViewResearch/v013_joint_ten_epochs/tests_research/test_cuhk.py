...
class CUHKTests(unittest.TestCase):
    ...
    def test_e0_epoch_scaled_schedule(self):
        ...
        for e in range(1,11):
            for s in range(212):
                index = (e-1)*212+s
                self.assertGreater(lr_value(index,'E0',10,212), 0)
                self.assertEqual(loss_weights(e,s,'E0',212)[1], 0)
        with self.assertRaises(ValueError):
            lr_value(2120,'E0',10,212)

    def test_baseline_verification_rejects_test_and_bad_steps(self):
        rows = [{'epoch':e,'batches':212,'validation':{'r1':50.+e,'mAP':40.+e}} for e in range(1,11)]
        result = dict(experiment='E0',seed=1,epochs=10,status='complete',config={'dataset':'CUHK-PEDES','input_resolution':[384,128]},
                      test_at_best_validation=None,history=rows,best_validation=dict(rows[-1]['validation'],epoch=10))
    ...
    def test_joint_training_requires_measured_reference_but_openai_initializer(self):
        ...
        cfg = json.loads((root/'configs/research_v013.json').read_text())
        baseline = dict(status='complete',experiment='E0',seed=1,epochs=10,
        ...
        wrong_duration = copy.deepcopy(baseline)
        wrong_duration['epochs'] = 5
        with patch('research.runner.read_json', side_effect=[copy.deepcopy(cfg),wrong_duration]):
            with self.assertRaises(ValueError):
                load_config('unused')
