...
class PersistenceTests(unittest.TestCase):
    def test_complete_best_persisted_and_sources_preserved(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            history=[{'epoch':e,'batches':212,'validation':{'r1':60.+e,'mAP':50.+e}} for e in range(1,11)]
            best=dict(history[-1]['validation'],epoch=10)
            result=dict(status='complete',experiment='E0',seed=1,epochs=10,
            ...
            payload=dict(model={'weight':torch.ones(2)},optimizer={},checkpoint_role='resumable',
                         next_epoch=11,next_step=0,history=history,best=best,best_checkpoint='best_epoch_010.pth')
            ...
            self.assertEqual(os.stat(destination/'checkpoints/best.pth').st_ino,
                             os.stat(destination/'checkpoints/best_epoch_010.pth').st_ino)
