...
RAM_ROOT = Path('/dev/shm/TBPS_ViewResearch_v013_20260907')
DISK_ROOT = Path('/root/autodl-tmp/TBPS_ViewResearch_v013_20260907')
...
def main():
    ...
    if last['next_epoch'] != 11 or last['next_step'] != 0 or last.get('checkpoint_role') != 'resumable':
        raise ValueError('Last checkpoint is not the complete resumable state')
    if 'optimizer' not in last or len(last['history']) != 10:
