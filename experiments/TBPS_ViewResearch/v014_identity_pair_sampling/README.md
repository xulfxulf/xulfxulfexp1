# V014: identity-pair sampling for the five-epoch soft-view model

## Hypothesis and comparison

Only change from retained V012 is main-batch sampling: existing view-aware
P160 K2 plans replace caption-pair DistributedSampler plans. Train from the
same OpenAI ViT-B/16 file for five epochs,seed1,with all V012 losses,learning
rates,augmentations,auxiliary supports and architecture unchanged. V013's
ten-epoch change is NOT included. No additional E0 is trained.
...
The direct retained method comparator is V012,whose selected validation
epoch5 has R1=73.33549499511719,mAP=65.94811248779297. The user permits focus
...
python launch_research.py --stage audit --config configs/research_v014.json \
  --record-dir /root/autodl-tmp/TBPS_ViewResearch_v014_20260907/audit_launch \
  --output-dir /dev/shm/TBPS_ViewResearch_v014_20260907/audit_run
# Require >=1 actual optimizer update across8 full-size four-GPU audit steps.
python launch_research.py --stage train --config configs/research_v014.json \
  --record-dir /root/autodl-tmp/TBPS_ViewResearch_v014_20260907/validation_launch \
  --output-dir /dev/shm/TBPS_ViewResearch_v014_20260907/validation_run
python run_research.py verify-result --result /dev/shm/TBPS_ViewResearch_v014_20260907/validation_run/result.json
python persist_completed_run.py --phase validation_run \
  --output-dir /root/autodl-tmp/TBPS_ViewResearch_v014_20260907/validation_run
```
