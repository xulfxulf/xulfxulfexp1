...
def disk_guard(directory, checkpoint_bytes, remaining_runs=1, compact_best=False):
    free = shutil.disk_usage(directory).free
    if compact_best:
        # AdamW last contains weights and two moments; best omits moments.
        # Budget two last files plus two compact best files during a transaction.
        needed = int(remaining_runs) * int(2.8 * checkpoint_bytes) + 2 * 1024**3
    else:
...
def selection_payload(saved):
    if not saved.get('config', {}).get('compact_best', False):
        return saved
    compact = {key: value for key, value in saved.items() if key != 'optimizer'}
    compact['checkpoint_role'] = 'selection_only'
    return compact
...
def guard_checkpoint_write(path, payload):
    if payload.get('config', {}).get('compact_best', False):
        # Recheck actual payload size before EACH atomic temporary write.
        needed = int(tensor_bytes(payload) * 1.02) + 16 * 1024**2 + 1024**3
        if shutil.disk_usage(Path(path).parent).free < needed:
            raise RuntimeError('Insufficient disk for atomic checkpoint write; old commit preserved')


def atomic_torch_save(path, payload):
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    guard_checkpoint_write(path, payload)
...
def save_checkpoint(paths, payload, generator, control_group):
    ...
    if rank() == 0:
        try:
            ...
            if saved.get('config', {}).get('compact_best', False):
                saved['checkpoint_role'] = 'resumable'
            ...
            if last is None:
                ...
                if best_path is not None:
                    ...
                    atomic_torch_save(version, selection_payload(saved))
