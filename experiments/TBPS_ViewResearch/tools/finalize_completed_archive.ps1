param([Parameter(Mandatory=$true)][ValidateSet('v013','v014','v015','v016')][string]$Version)
...
$partial = Join-Path $base 'method_completed_archive.tar.part'
$archive = Join-Path $base 'method_completed_archive.tar'
$run = Join-Path $base 'validation_run'
...
$expectedRemote = '/dev/shm/TBPS_ViewResearch_' + $Version + '_20260907/method_completed_archive.tar'
...
foreach ($entry in $entries) {
    if (-not $entry.StartsWith('validation_run/') -or $entry.Contains('\') -or
        ($entry -split '/') -contains '..') { throw 'Unexpected archive member' }
}
if ('validation_run/checkpoints/best.pth' -notin $entries -or
    'validation_run/checkpoints/last.pth' -notin $entries) { throw 'Missing checkpoints' }
Rename-Item -LiteralPath $partial -NewName 'method_completed_archive.tar'
