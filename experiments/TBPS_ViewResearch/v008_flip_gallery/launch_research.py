...
def main():
    ...
    p.add_argument("--stage", choices=["audit", "train"], required=True)
    ...
    for entry in Path('/proc').iterdir():
        ...
        if 'train.py' in names or ('train' in names and any(x in names for x in ('run_research.py', 'run_view4.py'))):
    ...
    command = [sys.executable, '-m', 'torch.distributed.run', '--standalone', '--nproc_per_node=4',
               str(root / 'run_research.py'), args.stage, '--config', str(Path(args.config).resolve()),
               '--output-dir', str(Path(args.output_dir).resolve())]
    if args.stage == 'audit':
        command.extend(['--steps', '4'])
