...
class Tiny(SoftViewCLIP):
    def __init__(self):
        ...
        self.shared_temperature = .07
        self.expert_queries = nn.Parameter(torch.zeros(3, 8))
...
class SoftThreeTests(unittest.TestCase):
    ...
    def test_zero_heads_equal_shared_embedding(self):
        ...
        v, r = model.fuse(image, p, torch.randn(6, 5, 8))
    ...
    def test_shared_gradient_and_head_only_decorrelation(self):
        ...
        patches = torch.randn(4, 5, 8, requires_grad=True)
        support_patches = torch.randn(4, 5, 8, requires_grad=True)
        batch = {"view_prob": p, "support_prob": p.roll(1, 1), "person_id": torch.arange(4)}
        ...
        self.assertIsNone(support_patches.grad)
        self.assertGreater(float(model.expert_queries.grad.norm()), 0)
    ...
    def test_zero_query_is_uniform_patch_pool(self):
        model = Tiny()
        patches = torch.randn(4, 7, 8)
        pooled, attention = model.pool_patches(patches)
        torch.testing.assert_close(attention, torch.full((4, 3, 7), 1 / 7))
        torch.testing.assert_close(pooled, patches.mean(1)[:, None].expand(-1, 3, -1))

    def test_retrieval_updates_patches_and_pool_queries(self):
        model = Tiny()
        h = torch.randn(4, 8, requires_grad=True)
        patches = torch.randn(4, 5, 8, requires_grad=True)
        probabilities = torch.softmax(torch.randn(4, 3), -1)
        v, _ = model.fuse(h, probabilities, patches)
        (v * torch.randn_like(v)).sum().backward()
        for p in (h, patches, model.expert_queries):
            self.assertIsNotNone(p.grad)
            self.assertGreater(float(p.grad.norm()), 0)

    def test_patch_path_is_required_and_gallery_excludes_cls(self):
        model = Tiny()
        h, dense = torch.randn(2, 8), torch.randn(2, 6, 8)
        model.encode_image = lambda images, return_dense: (h, dense)
        probabilities = torch.eye(3)[:2]
        expected = model.fuse(h, probabilities, dense[:, 1:])[0]
        actual = model.image_embedding(torch.zeros(2, 3, 4, 4), probabilities)
        torch.testing.assert_close(expected, actual)
        with self.assertRaises(ValueError):
            model.fuse(h, probabilities, dense[:, :0])
        with self.assertRaises(ValueError):
            model.fuse(h, probabilities[:1], dense[:, 1:])
