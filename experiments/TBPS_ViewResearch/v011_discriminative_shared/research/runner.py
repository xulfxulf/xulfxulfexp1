...
def load_config(path):
    ...
    if cfg["version"] != VERSION or cfg["dataset"] != "CUHK-PEDES":
        raise ValueError("This immutable version is the CUHK portrait soft3 experiment only")
    ...
    if (cfg.get("shared_objective") != "symmetric_temperature_scaled_crossview_nce" or
            cfg.get("shared_temperature") != .07):
        raise ValueError("V011 requires the fixed discriminative shared objective and tau=.07")
    ...
    if (baseline["status"], baseline["experiment"], baseline["seed"], baseline["epochs"]) != ("complete", "E0", 1, 5):
        raise ValueError("Measure the completed CUHK portrait E0 before continuing")
