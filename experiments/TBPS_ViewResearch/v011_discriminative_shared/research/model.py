...
def discriminative_shared_values(a, b, bank, anchor_ids, bank_ids, temperature):
    if (a.ndim != 2 or b.shape != a.shape or bank.ndim != 2 or bank.shape[1] != a.shape[1] or
            anchor_ids.shape != (len(a),) or bank_ids.shape != (len(bank),) or
            not math.isfinite(temperature) or temperature <= 0):
        raise ValueError("Invalid shared contrastive input")
    a, b, bank = [F.normalize(x.float(), dim=-1, eps=1e-8) for x in (a, b, bank)]
    positive = (a * b).sum(-1, keepdim=True) / temperature
    same_identity = anchor_ids[:, None].eq(bank_ids[None, :])
    values = []
    for query in (a, b):
        negative = (query @ bank.t() / temperature).masked_fill(same_identity, -torch.inf)
        logits = torch.cat((positive, negative), dim=1)
        values.append(temperature * (torch.logsumexp(logits, dim=1) - positive[:, 0]))
...
class SoftViewCLIP(ViewCLIP):
    def __init__(self, official, num_classes, seed, cfg):
        ...
        self.shared_temperature = cfg["shared_temperature"]
    ...
    def auxiliary(self, h, support_h, batch, selected, patches, support_patches):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # Same main images form an auxiliary negative bank, never a larger TBPS batch.
            bank = self.all_gather(h.float()) if dist.is_initialized() else h.float()
            bank_ids = self.all_gather(batch["person_id"]) if dist.is_initialized() else batch["person_id"]
            values = discriminative_shared_values(a, b, bank, batch["person_id"][selected],
                                                   bank_ids, self.shared_temperature)
