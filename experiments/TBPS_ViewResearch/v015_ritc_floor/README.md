# V015: smaller R-ITC target floor in the five-epoch soft-view model

## Hypothesis and comparison

Only training change from retained V014: R-ITC target floor0.01->0.001.
Keep V014's existing view-aware P160 K2 plans. Train from the same OpenAI
ViT-B/16 file for five epochs,seed1,with the same learning rates,augmentations,
auxiliary supports and architecture. This is NOT a continuation of V014.
V013's ten-epoch change is NOT included. No additional E0 is trained.

A read-only diagnosis used10 identical train-only PK batches at V014's
selected checkpoint,identical weights/dropout,zero optimizer updates.
All gradients were finite; N-ITC was identical. Smaller-floor gradient norms
were6.162x in the visual last block,4.955x in the text last block,2.178x
in residual heads and2.512x in queries. Mean gradient cosine was0.341/0.393
in visual/text last blocks. This motivates a controlled trial,not a claim
of improved retrieval or a guarantee of stability from initialization.
...
The direct retained method comparator is V014,whose selected validation
epoch5 has R1=69.9902572631836,mAP=67.43901062011719. V012 remains a separate
...
changed to0.001. For normalized same-PID q, R-ITC uses log(q+0.001),without
...
python launch_research.py --stage audit --config configs/research_v015.json \
  --record-dir /root/autodl-tmp/TBPS_ViewResearch_v015_20260907/audit_launch \
  --output-dir /dev/shm/TBPS_ViewResearch_v015_20260907/audit_run
# Require >=1 actual optimizer update across8 full-size four-GPU audit steps.
python launch_research.py --stage train --config configs/research_v015.json \
  --record-dir /root/autodl-tmp/TBPS_ViewResearch_v015_20260907/validation_launch \
  --output-dir /dev/shm/TBPS_ViewResearch_v015_20260907/validation_run
python run_research.py verify-result --result /dev/shm/TBPS_ViewResearch_v015_20260907/validation_run/result.json
python persist_completed_run.py --phase validation_run \
  --output-dir /root/autodl-tmp/TBPS_ViewResearch_v015_20260907/validation_run
```
