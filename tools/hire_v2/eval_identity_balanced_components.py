#!/usr/bin/env python
"""Offline component evaluation for HIRE-v2 v16.2.1 checkpoints."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate HIRE-v2 v16.2.1 identity-balanced components"
    )
...
def collect_representations(model, image_loader, text_loader):
    ...
    for pid, token_ids in text_loader:
        ...
        for key in keys:
            text_parts[key].append(
                F.normalize(encoded[key].float(), dim=-1).cpu()
            )
    ...
    return (
        text_repr,
        image_repr,
        torch.cat(query_pids),
        torch.cat(gallery_pids),
    )


def chunked_similarity(
    query, gallery, device, query_chunk, gallery_chunk
):
    output = torch.empty(
        query.shape[0], gallery.shape[0], dtype=torch.float32
    )
    ...
    for q_start in range(0, query.shape[0], query_chunk):
        ...
        for g_start in range(
            0, gallery_device.shape[0], gallery_chunk
        ):
            g_end = min(
                g_start + gallery_chunk, gallery_device.shape[0]
            )
            g = gallery_device[g_start:g_end]
...
def main():
    ...
    args.training = False
    logger = setup_logger(
        "HIRE-v2-identity-balanced-eval",
    ...
    actual = unwrap(model)
    if not getattr(
        actual, "is_hire_v2_identity_balanced_model", False
    ):
        raise RuntimeError(
            "the config does not build the v16.2.1 identity-balanced model"
        )
    ...
    output_json = cli.output_json or op.join(
        ...
        "hire_v2_identity_balanced_components.json",
    )
    ...
    logger.info(
        "saved component results to %s", output_json
    )
